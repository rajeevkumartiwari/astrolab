...
def plot_solar_system():
    ...
    AXIS_LIMIT_KM = 2e9  # or smaller if you want inner planets only

    ax.set_xlim(-AXIS_LIMIT_KM, AXIS_LIMIT_KM)
    ax.set_ylim(-AXIS_LIMIT_KM, AXIS_LIMIT_KM)
    ax.set_aspect('equal')  # preserves orbit shapes
    ...
    fig.patch.set_facecolor('black')  # Background outside plot area
    ax.set_facecolor('black')         # Background inside plot area
    ...
    while True:
        ...
        ax.scatter(0, 0, color='white', s=10, label='Barycenter')
        # ax.text(0, 0, 0, 'Barycenter', color='black')
        ax.scatter(log_val(sun_pos.x.value), log_val(sun_pos.y.value), color='yellow', s=200, label='Sun')
        ax.scatter(log_val(mercury_pos.x.value), log_val(mercury_pos.y.value), color='gray', s=8, label='Mercury')
        ax.scatter(log_val(venus_pos.x.value), log_val(venus_pos.y.value), color='orange', s=15, label='Venus')
        ax.scatter(log_val(earth_pos.x.value), log_val(earth_pos.y.value), color='blue', s=20, label='Earth')
        ax.scatter(log_val(moon_pos.x.value), log_val(moon_pos.y.value), color='lightgray', s=5, label='Moon')
        ax.scatter(log_val(mars_pos.x.value), log_val(mars_pos.y.value), color='red', s=15, label='Mars')
        ax.scatter(log_val(jupiter_pos.x.value), log_val(jupiter_pos.y.value), color='brown', s=100, label='Jupiter')
        ax.scatter(log_val(saturn_pos.x.value), log_val(saturn_pos.y.value), color='gold', s=80, label='Saturn')
        ax.scatter(log_val(uranus_pos.x.value), log_val(uranus_pos.y.value), color='lightblue', s=40, label='Uranus')
        # print("Neptune:", neptune_pos.x.value, neptune_pos.y.value, neptune_pos.z.value)
        ax.scatter(log_val(neptune_pos.x.value), log_val(neptune_pos.y.value), color='darkblue', s=40, label='Neptune')
